...
## Purpose
Define the requirements for managing products, including commission percentages and contribution types.

## Requirements

### Requirement: Product Commission Percentage Field
The `Product` entity MUST store a `commissionPercentage` as a non-null Decimal value (default 0). The field uses Decimal(7,4) precision to store raw percentage values (0-100 scale, not 0-1).

#### Scenario: Create product with commission percentage
- GIVEN an administrator submits a valid product creation request
- WHEN `commissionPercentage` is provided as a numeric value (e.g., 76.50)
...
The `Product` entity MUST store a `contributionType` as a non-null enum value of `REGULAR` or `UNICO` (default `REGULAR`). This field categorizes how the product is classified for contribution purposes.

#### Scenario: Create product with valid contribution type
- GIVEN an administrator submits a product with `contributionType` of `REGULAR` or `UNICO`
- WHEN the request is processed
- THEN the product is persisted with the provided contribution type

#### Scenario: Create product with invalid contribution type
- GIVEN an administrator submits a product with `contributionType` of an invalid value
- WHEN the request is validated
- THEN a 400 error is returned with a message listing valid values (REGULAR, UNICO)

#### Scenario: Update product contribution type
- GIVEN an existing product with `contributionType = REGULAR`
- WHEN an administrator updates it to `UNICO`
- THEN the product record reflects `UNICO` and an audit log entry is created

### Requirement: Product Form UI — Commission Fields
The `ProductForm` component MUST render inputs for `commissionPercentage` (numeric) and `contributionType` (select: REGULAR | UNICO). Both fields MUST be included in the Zod validation schema.
...
Mapping rules:
- `PRODUCTO` column → match by `Product.name` (case-insensitive, cross-referenced with company name)
- `APORTE` column → `contributionType` (`REGULAR` or `UNICO`)
- `% COMISIONAL PAGO LIQUIDACION` column → `commissionPercentage` (strip `%` and parse as Decimal; store as 0-100 raw value, NOT divided by 100)

#### Scenario: Seed updates matched products
- GIVEN the CSV contains a row with `PRODUCTO = "ProductoX"`, `APORTE = "REGULAR"`, `% COMISIONAL PAGO LIQUIDACION = "76.50%"`
- WHEN the seed script runs
- THEN the product with name `ProductoX` is updated to `commissionPercentage = 76.50` and `contributionType = REGULAR`
...
- THEN `commissionPercentage` is stored as `12.5` (NOT `0.125`)

## Data Mapping Rules
| CSV Column | Product Field | Transformation |
|------------|---------------|----------------|
| PRODUCTO | name | Match case-insensitive with company context |
| APORTE | contributionType | "REGULAR" -> REGULAR, "UNICO" -> UNICO |
| % COMISIONAL PAGO LIQUIDACION | commissionPercentage | Remove "%", parse as Decimal (0-100 raw scale) |
