...
- [x] 1.4.1 Add Prisma data migration `20260903120000_seed_aba_mfund_report_permissions` so deploy (`prisma migrate deploy`) upserts the `ABA_MFUND` and `LEADS_ANALYTICS` catalog rows and enables Performance Leader + Business Leader for both — same pattern as Producción Real. Seeds alone are not enough because pipelines do not run `prisma db seed`.
