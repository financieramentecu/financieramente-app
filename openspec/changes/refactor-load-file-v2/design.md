...
flowchart TD
    ...
    V_HAY_LAG -- SÍ --> V_REC_LAG[Actualizar LAG antiguo a SYNC<br/>& Crear Nuevo como SYNC]:::state_sync
    ...
    GET_CONF_V --> SAVE_V[(Guardar BD:<br/>Estado, Contrato,<br/>% Com, % Desc)]:::db
...
## Technical Strategy
- Refactor `process-batch.service.ts` to implement the decision tree.
- Use Prisma transactions where necessary (especially when updating an old LAG and creating a new SYNCHRONIZED record simultaneously).