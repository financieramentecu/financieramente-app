...
**Choice**: `prisma/seeds/reset-future-payments-to-sin-fondear.ts`, idempotent, `--dry-run` flag (log counts, no writes). Step 1: payments `where: { status: FONDEADO, dateAnchored: { gte: startOfTomorrowBogota } }` (funded date strictly after today) → per-row update in chunked transactions: `{ status: SIN_FONDEAR, expectedDate: expectedDate ?? dateAnchored, dateAnchored: null }`. Per-row (not `updateMany`) because the schedule must be preserved column-to-column: legacy rows store the scheduled date in `dateAnchored` with `expectedDate` null, and the cron/visibility rules filter by `expectedDate`. Step 2: backfill `business.updateMany` for every business with ≥1 `EN_CARTERA` payment (subquery via `findMany distinct idBusiness`) → `status: 'CARTERA'`. Re-run is a no-op (where-clauses self-exclude).
**Rationale**: Boundary `dateAnchored > today Bogota` per decision log round 4 (a future funded date marks a legacy pre-funded row that was never actually funded); copying into `expectedDate` keeps those payments visible to the cron after reset; EN_CARTERA never matched; idempotent by construction. Step 1 audits with `PAYMENT_MIGRATION_RESET` (not `PAYMENT_CRON_FUNDED` — the trail must say what actually happened).
