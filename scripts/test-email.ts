...
import { SendGridEmailService } from '../src/infrastructure/email/sendgrid/SendGridEmailService'
import { SendGridConfig } from '../src/infrastructure/email/sendgrid/SendGridConfig'
import { EmailAddress } from '../src/domain/email/value-objects/EmailAddress'
import { EmailSubject } from '../src/domain/email/value-objects/EmailSubject'
import { Email } from '../src/domain/email/entities/Email'
...
async function sendTraditionalEmail(to: string): Promise<void> {
    ...
    try {
        ...
        // Crear Value Objects usando el patrón factory
        const toAddressResult = EmailAddress.create(to)
        if (toAddressResult instanceof Error) {
            logError(`Email de destino inválido: ${toAddressResult.message}`)
            return
        }

        const fromAddressResult = EmailAddress.create(SendGridConfig.getFromEmail())
        if (fromAddressResult instanceof Error) {
            logError(`Email de origen inválido: ${fromAddressResult.message}`)
            return
        }

        const subjectResult = EmailSubject.create('🧪 Email de Prueba - Financieramente')
        if (subjectResult instanceof Error) {
            logError(`Asunto inválido: ${subjectResult.message}`)
            return
        }
...
async function main() {
    ...
    if (args.length > 0) {
        ...
        if (type === 'templated') {
            const templateId =
                args[2] || process.env.SENDGRID_TEMPLATE_ID || ''
            if (!templateId) {
                logError(
        ...
        logInfo('Ejemplo:')
        console.log(
            `  npm run test:email ${SendGridConfig.getFromEmail()}`
        )
        console.log()
    }
