...
# SSH Tunnel Helper Script for Production Database Access
# This script creates an SSH tunnel to access the Production PostgreSQL database locally
...
# Function to get Production Droplet IP
get_prod_ip() {
	# 1) From local file
	if [ -f "$PROD_IP_FILE" ]; then
		cat "$PROD_IP_FILE"
		return 0
	fi

	# 2) From Terraform output (if available)
	if [ -d "terraform" ]; then
		local prod_ip
		prod_ip=$(cd terraform && terraform output -raw prod_droplet_ip 2>/dev/null || true)
		if [ -n "$prod_ip" ]; then
			echo "$prod_ip"
			return 0
		fi
	...
	print_error "Could not resolve Production Droplet IP" >&2
	print_status "Set PROD_DROPLET_IP, or create $PROD_IP_FILE, or ensure terraform output works" >&2
	return 1
}
...
create_tunnel() {
	...
	print_status "Creating SSH tunnel to Production database..."
	print_status "Local port: $LOCAL_PORT"
	print_status "Remote: $prod_ip:$REMOTE_PORT"
	print_status "SSH key: $SSH_KEY"

	# Create the tunnel in background
	ssh -f -N -L $LOCAL_PORT:localhost:$REMOTE_PORT root@$prod_ip -i $SSH_KEY
	...
	if lsof -Pi :$LOCAL_PORT -sTCP:LISTEN -t >/dev/null; then
		...
		echo "  Database: financieramente_prod"
		echo "  Username: financieramente_user"
		echo "  Password: [from GitHub Secrets: POSTGRES_PASSWORD]"
		echo ""
		print_status "For your .env.local file:"
		echo "  DATABASE_URL=\"postgresql://financieramente_user:[PASSWORD]@localhost:$LOCAL_PORT/financieramente_prod\""
		echo ""
		print_status "To stop the tunnel, run: ./scripts/db-tunnel-prod.sh stop"
	else
...
show_status() {
	if lsof -Pi :$LOCAL_PORT -sTCP:LISTEN -t >/dev/null; then
		...
		print_status "Port $LOCAL_PORT is forwarding to Production database"
	else
...
test_connection() {
	...
	if psql -h localhost -p $LOCAL_PORT -U financieramente_user -d financieramente_prod -c "SELECT 1;" >/dev/null 2>&1; then
...
main() {
	case "${1:-start}" in
		start)
			...
			# Get Production IP from environment variable or file
			if [ -n "$PROD_DROPLET_IP" ]; then
				PROD_IP="$PROD_DROPLET_IP"
			else
				PROD_IP=$(get_prod_ip) || exit 1
			fi

			if ! check_existing_tunnel; then
				create_tunnel "$PROD_IP"
			fi
			;;
		stop)
			stop_tunnel
		...
		*)
			...
			echo ""
			echo "Commands:"
			echo "  start   - Create SSH tunnel to Production database (default)"
			...
			echo "  PROD_DROPLET_IP - IP address of Production Droplet (optional)"
			echo ""
			echo "Examples:"
			echo "  $0                    # Start tunnel"
			...
			echo "  PROD_DROPLET_IP=1.2.3.4 $0 start  # Use specific IP"
			exit 1
			;;
	esac
}
