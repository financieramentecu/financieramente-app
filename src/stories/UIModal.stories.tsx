...
const meta: Meta<typeof Modal> = {
	...
	parameters: {
		...
		docs: {
			description: {
				component:
					'Sistema completo de modales con variantes para alertas, confirmaciones y formularios. Cumple con estándares WCAG AA de accesibilidad.',
			},
		},
	},
	argTypes: {
	...
	render: (args) => {
		...
		return (
			<Modal
				...
				trigger={<Button onClick={() => setOpen(true)}>Abrir Modal</Button>}
			>
				{args.children}
			</Modal>
		)
	...
	args: {
		...
		children: (
			<div className="py-4 space-y-4">
				<p className="text-sm">
					Este es un modal grande que puede contener más información.
				</p>
...
export const WithoutCloseButton: Story = {
	args: {
		...
		children: (
			<div className="py-4">
				<p className="text-sm">
					Solo se puede cerrar haciendo clic fuera del modal o presionando
					Escape.
				</p>
			</div>
		),
	...
	render: () => {
		...
		return (
			<div className="space-y-4">
				<div className="grid grid-cols-2 gap-4">
					<Button onClick={() => setModalOpen(true)}>Modal Básico</Button>
					<Button onClick={() => setAlertOpen(true)}>Alerta Info</Button>
					<Button onClick={() => setConfirmOpen(true)}>Confirmación</Button>
					<Button onClick={() => setFormOpen(true)}>Formulario</Button>
				</div>
