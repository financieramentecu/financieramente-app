...
const meta: Meta = {
	...
	parameters: {
		...
		docs: {
			description: {
				component:
					'Ejemplos prácticos de cómo usar el sistema modular de tablas con diferentes configuraciones y casos de uso.',
			},
		},
	},
}
...
export const BasicUsage: StoryObj = {
	render: () => {
		...
		const userColumns = TableConfigs.users
			.columns as unknown as ColumnDef<UsersRow>[]
		...
		return (
			<div className="space-y-4">
				...
				<p className="text-sm text-muted-foreground">
					Usa TableConfigs para configuraciones comunes como usuarios,
...
export const CustomColumns: StoryObj = {
	render: () => {
		...
		const customColumns = [
			createColumnDefs.text('productName', 'Nombre del Producto', {
				searchable: true,
			}),
		...
		return (
			<div className="space-y-4">
				...
				<p className="text-sm text-muted-foreground">
					Crea columnas personalizadas usando createColumnDefs y funciones de
					renderizado.
				</p>
	...
	render: () => {
		...
		return (
			<div className="space-y-4">
				...
				<p className="text-sm text-muted-foreground">
					Usa el hook useTableModule para crear componentes de tabla
					reutilizables.
				</p>
...
export const CustomActions: StoryObj = {
	render: () => {
		...
		const columns = [
			createColumnDefs.text('name', 'Nombre del Documento', {
				searchable: true,
			}),
			...
			createColumnDefs.badge('status', 'Estado', {
				getVariant: (status: unknown) =>
					String(status) === 'Activo' ? 'default' : 'secondary',
			}),
...
export const AllConfigurations: StoryObj = {
	render: () => {
		...
		const userColumns = TableConfigs.users.columns as unknown as ColumnDef<
		...
		const productColumns = TableConfigs.products
			.columns as unknown as ColumnDef<(typeof productData)[number]>[]
			...
			<div className="space-y-8">
				<div>
					...
					<p className="text-muted-foreground mb-6">
						Ejemplos de todas las configuraciones disponibles en el sistema
						modular de tablas.
					</p>
				</div>
				...
				<div>
					<h3 className="text-lg font-semibold mb-4">
						1. Tabla de Usuarios (Pre-configurada)
					</h3>
					<TableModule
						data={userData}
						columns={userColumns}
				...
				<div>
					<h3 className="text-lg font-semibold mb-4">
						2. Tabla de Productos (Pre-configurada)
					</h3>
					<TableModule
						data={productData}
