  ...
  for (const item of ALL_MENU_ITEMS) {
    ...
    if (item.title === "Negocios" && permissions.negocios.list) {
      const subItems = item.subItems?.filter((subItem) => {
        if (subItem.title === "Listar Negocios") return permissions.negocios.list
        if (subItem.title === "Crear Negocio") return permissions.negocios.create
        if (subItem.title === "Cancelar Negocio") return permissions.negocios.cancel
        return false
      })
    ...
    if (item.title === "Cargas") {
      if (permissions.cargas.cargaMasiva || permissions.cargas.historial) {
        const subItems = item.subItems?.filter((subItem) => {
          if (subItem.title === "Carga Masiva") return permissions.cargas.cargaMasiva
    ...
    if (item.title === "Liquidaciones") {
      if (permissions.liquidaciones.preliquidacion || permissions.liquidaciones.liquidacion) {
        const subItems = item.subItems?.filter((subItem) => {
          if (subItem.title === "Preliquidación") return permissions.liquidaciones.preliquidacion
          if (subItem.title === "Liquidación") return permissions.liquidaciones.liquidacion
          return false
        })
    ...
    if (item.title === "Reportes") {
      if (permissions.reportes.all || permissions.reportes.business || permissions.reportes.personal) {
        const subItems = item.subItems?.filter((subItem) => {
          if (subItem.title === "Todos los Reportes") return permissions.reportes.all
          if (subItem.title === "Reportes de Negocio") return permissions.reportes.business
          if (subItem.title === "Mis Reportes") return permissions.reportes.personal
          return false
        })
 ...
 */
export function getRedirectUrlByRole(role: UserRole | string | null | undefined): string {
  if (role === UserRole.AGENTE) {
