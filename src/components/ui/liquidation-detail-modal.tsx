...
import type { LiquidationDetail } from "@/types/liquidation"
      ...
      >
        <DialogHeader className="flex flex-row items-center justify-between pb-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <Badge className={`${getStatusColor(liquidation.status)} text-sm px-3 py-1 rounded-full`}>
              {liquidation.status}
            </Badge>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-6 py-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="flex items-center gap-3 mb-4">
                <Avatar className="h-16 w-16">
                  ...
                  <AvatarFallback className="bg-blue-200 text-blue-800">
                    {liquidation.client.name.split(' ').map(n => n[0]).join('')}
                  </AvatarFallback>
                </Avatar>
                <div>
                  ...
                  <p className="text-blue-900 text-sm">
                    {liquidation.client.identificationType} {liquidation.client.identification}
                  </p>
              ...
              <div className="border-t pt-3">
                <h4 className="font-bold text-blue-900 mb-2 text-sm">Información cliente</h4>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <h4 className="font-bold text-blue-900 mb-3 text-sm">Información del agente</h4>
              <div className="flex items-center gap-3 mb-4">
                <Avatar className="h-16 w-16">
                  <AvatarImage 
                  ...
                  <AvatarFallback className="bg-gray-200 text-blue-900">
                    {liquidation.agent.name.split(' ').map(n => n[0]).join('')}
                  </AvatarFallback>
                </Avatar>
                <div>
                  <p className="font-bold text-blue-900">{liquidation.agent.name}</p>
                  <p className="text-gray-500 text-sm">{liquidation.agent.role}</p>
                </div>
              </div>
              <div className="border-t pt-3">
                <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div className="text-4xl font-bold text-blue-900 text-center">
            $ {liquidation.amount.toLocaleString('es-CO')} {liquidation.currency.toLowerCase()}
          </div>
            ...
            <div className="border border-gray-200 rounded-lg p-4 bg-white">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-gray-500 text-xs">{liquidation.insurance.code}</p>
                    <p className="font-bold text-blue-900">{liquidation.insurance.name}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-gray-500 text-xs">Perioricidad</p>
                  <p className="text-blue-900 font-semibold">{liquidation.product.periodicity || liquidation.product.term}</p>
                  <p className="text-gray-500 text-xs mt-1">Moneda</p>
                  <p className="text-blue-900 font-semibold">{liquidation.currency}</p>
                </div>
              </div>
            </div>
            ...
            <div className="border border-gray-200 rounded-lg p-4 bg-white">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="font-bold text-blue-900">{liquidation.product.name}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-blue-900 font-semibold">{liquidation.product.term}</span>
              </div>
            </div>
          </div>
        </div>
