...
describe('GET /api/negocios', () => {
	...
	describe('Búsqueda Unificada', () => {
		...
		it('debe buscar con coincidencia exacta (exactMatch: true)', async () => {
			const mockSession = {
				user: {
					email: 'admin@example.com',
				},
			...
			const mockBusinesses: never[] = []
			const mockEntities: never[] = []
			...
			mockGetCurrentUserByEmail.mockResolvedValue(mockAdminUser as never)
			mockPrismaCount.mockResolvedValue(0)
			mockPrismaFindMany.mockResolvedValue(mockBusinesses as never)
			mockPrismaBusinessListToEntities.mockReturnValue(mockEntities as never)

			const request = new Request(
				'http://localhost:3000/api/negocios?search=PN0001234&exactMatch=true'
			)
			await GET(request)
