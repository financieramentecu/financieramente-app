...
export async function GET(
    ...
    try {
        ...
        const registrosFormateados = registros.map((r) => {
            ...
            if (r.business?.productPercentajeCommision?.productPercentajeCommisionCategories) {
                r.business.productPercentajeCommision.productPercentajeCommisionCategories.forEach((cat) => {
            ...
            const generalDescuento = generalBruta.sub(generalBruta.mul(DESCUENTO_POR_DEFECTO))
            const agenciaDescuento = agenciaBruta.sub(agenciaBruta.mul(DESCUENTO_POR_DEFECTO))
            const liderDescuento = liderBruta.sub(liderBruta.mul(DESCUENTO_POR_DEFECTO))
            const coachDescuento = coachBruta.sub(coachBruta.mul(DESCUENTO_POR_DEFECTO))
        ...
        const response = {
            ...
            resumen: {
                ...
                sincronizados: registros.filter((r) => r.status === 'SINCRONIZADO').length,
                rezagados: registros.filter((r) => r.status === 'LAG').length,
                totalComision: registrosFormateados.reduce((sum, r) => sum + r.comision, 0),
                totalGeneral: registrosFormateados.reduce((sum, r) => sum + r.generalDescuento, 0),
                totalAgencia: registrosFormateados.reduce((sum, r) => sum + r.agenciaDescuento, 0),
                totalLider: registrosFormateados.reduce((sum, r) => sum + r.liderDescuento, 0),
                totalCoach: registrosFormateados.reduce((sum, r) => sum + r.coachDescuento, 0),
            },
        }
