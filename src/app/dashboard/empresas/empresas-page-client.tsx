...
import { EmpresasTableSection } from '@/features/empresas/components/empresas-table'
import { useEmpresas } from '@/features/empresas/hooks/use-empresas'
...
/**
 * Skeleton para la tabla de empresas
 */
...
/**
 * Componente Cliente para la Página de Listado de Empresas
 */
export function EmpresasPageClient() {
	...
	const { state, refetch } = useEmpresas({
	...
	const { deleteEmpresa, deleteState } = useEmpresaMutations()
	...
	// Manejar agregar nueva empresa
	const handleAddEmpresa = useCallback(() => {
		router.push('/dashboard/empresas/create')
	}, [router])

	// Manejar editar empresa
	const handleEditEmpresa = useCallback(
		(empresa: Empresa) => {
			router.push(`/dashboard/empresas/editar/${empresa.idCompany}`)
		},
		[router]
	)

	// Manejar eliminar empresa
	...
	const handleConfirmDelete = useCallback(async () => {
		...
		await deleteEmpresa(empresaToDelete.idCompany)
	...
	}, [empresaToDelete, deleteEmpresa, deleteState, refetch])
	...
	return (
		<div className="space-y-6">
			...
			{showFullSkeleton ? (
				...
				<EmpresasTableSection
					data={state.status === 'success' ? state.data.empresas : []}
