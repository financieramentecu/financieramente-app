...
function ProductOriginsSection() {
  ...
  const [selectedOrigin, setSelectedOrigin] = useState<ProductOrigin | null>(null)
  ...
  const handleSubmit = async (data: ProductOriginFormData) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.details || result.error || "Error al guardar origen de producto"
        )
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setIsSubmitting(true)
      const response = await fetch(`/api/admin/product-origins/${selectedOrigin.idOrigin}`, {
        method: "DELETE",
      })
  ...
  return (
    <div className="space-y-6">
      ...
      <CrudModal
        ...
        title={mode === "create" ? "Crear Origen de Producto" : "Editar Origen de Producto"}
        description={
          mode === "create"
...
function ClientOriginsSection() {
  ...
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false)
  const [selectedOrigin, setSelectedOrigin] = useState<ClientOrigin | null>(null)
  const [mode, setMode] = useState<"create" | "edit">("create")
  const [isSubmitting, setIsSubmitting] = useState(false)
  ...
  return (
    <div className="space-y-6">
      ...
      <CrudModal
        open={isModalOpen}
        onOpenChange={setIsModalOpen}
        title={mode === "create" ? "Crear Origen de Cliente" : "Editar Origen de Cliente"}
        description={
          mode === "create"
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Gestiona los orígenes de productos y clientes utilizados en el sistema
          </p>
        </div>
