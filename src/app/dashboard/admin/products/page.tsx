...
export default function ProductsAdminPage() {
  ...
  const handleSubmit = async (formData: Record<string, unknown>) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(result.details || result.error || "Error al guardar producto")
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setIsSubmitting(true)
      const response = await fetch(`/api/admin/products/${selectedProduct.idProduct}`, {
        method: "DELETE",
      })
  ...
  const columns: CrudTableColumn<Product>[] = [
    ...
    },
    {
      key: "name",
      header: "Nombre",
      cellRenderer: (value) => <span className="font-medium">{value as string}</span>,
    },
    {
      key: "company",
      header: "Compañía",
      cellRenderer: (_, row) => (
        <span className="text-sm text-muted-foreground">{row.company?.name ?? "—"}</span>
      ),
    },
    {
      key: "description",
      ...
      cellRenderer: (value) =>
        value ? (
          <span className="text-sm text-muted-foreground line-clamp-2">{value as string}</span>
        ) : (
  ...
  const fields: CrudModalField[] = useMemo(
    () => [
      ...
      },
      {
        ...
        label: "Activo",
        type: "switch",
        description: "Desactiva el producto para que no esté disponible en nuevas gestiones.",
      },
    ],
    [companyOptions]
  )
