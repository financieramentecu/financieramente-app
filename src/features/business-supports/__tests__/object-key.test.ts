...
describe('buildComprobanteKey', () => {
  ...
  it('builds a key matching the expected format', () => {
    ...
    expect(key).toBe(`${PREFIX}/negocios/${CONTRACT}/comprobantes/${CONTRACT}-${ts}-${FIXED_UUID}.jpg`)
  })
  ...
  it('includes contract in directory and filename', () => {
    const key = buildComprobanteKey({
      prefix: PREFIX,
  ...
  it('uses timestamp from now param', () => {
    ...
    const key1 = buildComprobanteKey({ prefix: PREFIX, contract: CONTRACT, ext: 'jpg', now: now1, uuid: FIXED_UUID })
    const key2 = buildComprobanteKey({ prefix: PREFIX, contract: CONTRACT, ext: 'jpg', now: now2, uuid: FIXED_UUID })
  ...
  it('uses uuid param in the filename', () => {
    ...
    const key1 = buildComprobanteKey({ prefix: PREFIX, contract: CONTRACT, ext: 'jpg', now: FIXED_NOW, uuid: uuid1 })
    const key2 = buildComprobanteKey({ prefix: PREFIX, contract: CONTRACT, ext: 'jpg', now: FIXED_NOW, uuid: uuid2 })
