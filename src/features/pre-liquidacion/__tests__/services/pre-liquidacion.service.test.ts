		...
		expect(prisma.settlementCommission.findMany).toHaveBeenCalledWith(
			expect.objectContaining({
				where: {
					idFileImport: 1,
					status: {
						in: ['SYNCHRONIZED', 'PRE-SETTLED', 'SETTLED', 'LAG'],
					},
				},
...
describe('liquidarRegistros', () => {
	...
	it('does not set FileImport SETTLED when SYNCHRONIZED rows still exist', async () => {
		vi.mocked(prisma.settlementCommission.findMany).mockResolvedValue([
			mockCommissionRow(1),
			mockCommissionRow(2),
			mockCommissionRow(3),
	...
	it('does not set FileImport SETTLED when 0 SYNCHRONIZED but PRE-SETTLED remain', async () => {
		vi.mocked(prisma.settlementCommission.findMany).mockResolvedValue([
			mockCommissionRow(1),
		] as any)
		vi.mocked(prisma.settlementCommission.updateMany).mockResolvedValue({
...
describe('rezagarRegistros', () => {
	...
	it('sets FileImport as SETTLED when lagging last records and returns fileCompleted: true', async () => {
		vi.mocked(prisma.settlementCommission.updateMany).mockResolvedValue({
			count: 3,
		})
