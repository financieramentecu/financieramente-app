...
describe('useOriginDonut', () => {
  it('returns idle state when sessionStatus is loading and no nodes', () => {
    setupMocks({ sessionStatus: 'loading', nodes: [], selectedUserIds: [] })
    const { result } = renderHook(() => useOriginDonut())
    // While session is loading, hook is in loading state (mirrors useMsBarChart)
    expect(result.current.status).toBe('loading')
  })

  it('returns idle state initially when nodes list is empty and session not yet resolved', () => {
    setupMocks({
      nodes: [],
      selectedUserIds: [],
      sessionUserId: null,
  ...
  it('transitions loading → success on happy path (MS Junior: no hierarchy nodes)', async () => {
    ...
    const success = result.current as AsyncSuccessState<OriginDonutSlice[]>
    expect(success.data).toHaveLength(2)
    // Each slice should have percentage and fill set by aggregateOriginDonut
  ...
  it('passes aggregated data with correct percentage sum (~100)', async () => {
    ...
    global.fetch = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        data: [
          { originId: 1, originName: 'A', currencyId: 2, currencyName: 'USD', currencySymbol: 'USD', count: 30 },
          { originId: 2, originName: 'B', currencyId: 1, currencyName: 'COP', currencySymbol: 'COP', count: 70 },
        ],
      }),
    } as Response)
  ...
  it('re-fetches when selectedUserIds changes', async () => {
    ...
    const { result, rerender } = renderHook(() => useOriginDonut())
  ...
  it('does not apply stale response after unmount (cancelled flag)', async () => {
    ...
    const { result, unmount } = renderHook(() => useOriginDonut())
