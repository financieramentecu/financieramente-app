...
import { StatusDonutTooltip } from './StatusDonutTooltip'
import { StatusDonutLegend } from './StatusDonutLegend'
import type { AsyncState } from '@/features/shared/types/async-state.types'
import type { StatusDonutSlice } from '../types/production-kpi.types'

interface StatusDonutChartProps {
  readonly chartState: AsyncState<StatusDonutSlice[]>
  readonly trmRate: number | null
}

/** Internal skeleton shown during loading or idle state. */
function StatusDonutSkeleton() {
  return (
...
/**
 * Pure renderer for the Status Donut chart.
 ...
 */
export function StatusDonutChart({ chartState, trmRate }: StatusDonutChartProps) {
  if (chartState.status === 'loading' || chartState.status === 'idle') {
    return <StatusDonutSkeleton />
  }

  if (chartState.status === 'error') {
    return (
      <div className="rounded-xl border bg-card p-4">
        <p className="text-sm text-destructive">Error al cargar la distribución por estado</p>
      </div>
    )
  }
  ...
  return (
    <div
      role="img"
      aria-label="Distribución de negocios por estado"
    ...
    >
      <ResponsiveContainer width="100%" height={320}>
        <PieChart>
          ...
          <Tooltip content={<StatusDonutTooltip trmRate={trmRate} />} />
        </PieChart>
      </ResponsiveContainer>
      <StatusDonutLegend slices={slices} />
    </div>
  )
}
