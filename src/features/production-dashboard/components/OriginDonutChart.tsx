...
/**
 * Pure renderer for the Origin Donut chart.
 ...
 */
export function OriginDonutChart({ chartState, trmRate }: OriginDonutChartProps) {
  if (chartState.status === 'loading' || chartState.status === 'idle') {
    return <OriginDonutSkeleton />
  }

  if (chartState.status === 'error') {
    return (
      <div className="rounded-xl border bg-card p-4">
        <p className="text-sm text-destructive">Error al cargar la distribución por origen</p>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <OriginDonutLegend slices={slices} />
    </div>
  )
}
