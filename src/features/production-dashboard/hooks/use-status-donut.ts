...
import type { StatusDonutRaw, StatusDonutSlice } from '../types/production-kpi.types'
...
/**
 * Fetches status distribution data from /api/production-dashboard/by-status,
 * aggregates into StatusDonutSlice[] and returns AsyncState<StatusDonutSlice[]>.
 *
 * Mirrors the auth/hierarchy gating pattern from useCompanyDonut:
 ...
 *
 * @param trmRate  TRM exchange rate for COP → USD conversion. Null when unavailable.
 */
export function useStatusDonut(trmRate: number | null): AsyncState<StatusDonutSlice[]> {
  const { nodes, selectedUserIds } = useHierarchySelection()
  const { appliedFilters } = useDashboardFilter()
  const { data: session, status: sessionStatus } = useSession()

  const [rawData, setRawData] = useState<StatusDonutRaw[] | null>(null)
    ...
    async function fetchDonut() {
      try {
        ...
        const response = await fetch(
          `/api/production-dashboard/by-status?${params.toString()}`,
        ...
        if (!response.ok) {
          setFetchError('Error al obtener distribución por estado')
          setFetchStatus('error')
          return
        }

        const body = (await response.json()) as ApiResponse<StatusDonutRaw[]>
        ...
        if ('error' in body || body.data === null) {
          setFetchError(
            (body as { data: null; error: string }).error ??
              'Error al obtener distribución por estado'
          )
          setFetchStatus('error')
          return
        }
  ...
  const data = aggregateStatusDonut(rawData, trmRate)
