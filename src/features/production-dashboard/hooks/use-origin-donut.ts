...
/**
 * Fetches origin distribution data from /api/production-dashboard/by-origin,
 * aggregates into OriginDonutSlice[] and returns AsyncState<OriginDonutSlice[]>.
 *
 * Mirrors the auth/hierarchy gating pattern from useMsBarChart:
 ...
 */
export function useOriginDonut(): AsyncState<OriginDonutSlice[]> {
  const { nodes, selectedUserIds } = useHierarchySelection()
  const { appliedFilters } = useDashboardFilter()
  const { data: session, status: sessionStatus } = useSession()

  const [rawData, setRawData] = useState<OriginDonutRaw[] | null>(null)
  ...
  useEffect(() => {
    ...
    async function fetchDonut() {
      try {
        ...
        if (!response.ok) {
          setFetchError('Error al obtener distribución por origen del cliente')
          setFetchStatus('error')
          return
        }
        ...
        if ('error' in body || body.data === null) {
          setFetchError(
            (body as { data: null; error: string }).error ??
              'Error al obtener distribución por origen del cliente'
          )
          setFetchStatus('error')
          return
        }
  ...
  const data = aggregateOriginDonut(rawData)
